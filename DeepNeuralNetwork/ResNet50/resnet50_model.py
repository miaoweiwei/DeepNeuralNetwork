...
def conv2D(A_prev, filters, k_size, strides, padding, name):
    """ 卷积
    :param A_prev: 输入数据 [batch_size, height, width, kernels]
    :param filters: 输出通道数
    :param k_size: kernel 尺寸
    :param strides: 卷积的步长
    :param padding:卷积的模式
    :param name: 该层的名字
    :return:
    ...
    w_shape = (k_size[0], k_size[1], in_C, filters)  # 卷积核的尺寸[宽，高，输入通道，输出通道]
    b_shape = (1, 1, 1, filters)  # 偏执

    W = get_weights(shape=w_shape, name=name + '_W')  # 定义 weight
    b = get_bias(shape=b_shape, name=name + '_b')  # 定义偏执

    strides = [1, strides[0], strides[1], 1]  # 卷积的步长
...
def batch_norm(X, name):
    """ 批次归一化
    :param X:输入数据 [batch_size, height, width, kernels]
    :param name:
    :return:
    """
    # 在 0,1,2 维度上计算均值和方差 keep_dims: 是否保持维度
    m_, v_ = tf.nn.moments(X, axes=[0, 1, 2], keep_dims=False)
    beta_ = tf.zeros(X.shape.as_list()[-1])
    gamma_ = tf.ones(X.shape.as_list()[-1])
...
def identity_block(X, f, filters, stage, block):
    """使用快捷方式路径实现ResNet标识块 经过3个Conv Layers
    :param X:input tensor of shape (m, in_H, in_W, in_C)
    :param f:中间层过滤器的大小
    :param filters:3层过滤器数量的元组
    :param stage:用于命名图层
    :param block:用于命名图层
    :return:
    """
...
def convolutional_block(X, f, filters, stage, block, s=2):
    """ 使用快捷方式路径实现ResNet卷积块 经过3个不同大小的Conv图层
    :param X:input tensor of shape (m, in_H, in_W, in_C)
    :param f: 中间层过滤器的大小
    :param filters: 3层过滤器数量的元组
    :param stage:用于命名图层
    :param block:用于命名图层
    :param s:在第一层卷积块中使用的步幅
    :return:
    """
    conv_name = 'res' + str(stage) + block + '_branch'
    bn_name = 'bn' + str(stage) + block + '_branch'
